...
if not os.path.exists(LOG_PATH): os.mkdir(LOG_PATH)
shutil.copy('kfold_2016.py', LOG_PATH)
...
# k=5 k-fold
kfold_test_idx = np.load(os.path.join(ROOT, 'data/graph/PDBbind2016/kfold_idx', '%d.npy'%args.fold))
kfold_train_idx = [i for i in range(4056)]
for i in kfold_test_idx: kfold_train_idx.remove(i) # 4056-811.2
adj_all = torch.cat([adj_train, adj_test], dim=0) # 4056
feat_all = torch.cat([feat_train, feat_test], dim=0)
label_all = torch.cat([label_train, label_test], dim=0)
adj_train = adj_all[kfold_train_idx] # 4056
