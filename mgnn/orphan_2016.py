...
parser.add_argument('--protein', type=int, default=0, help='protein index number.')
...
LOG_PATH = os.path.join(ROOT, 'log', args.dataset+'_p'+str(args.protein))
if not os.path.exists(MODEL_PATH): os.mkdir(MODEL_PATH)
if not os.path.exists(LOG_PATH): os.mkdir(LOG_PATH)
if not os.path.exists(LOG_PATH): os.mkdir(LOG_PATH)
shutil.copy('orphan_2016.py', LOG_PATH)
...
save_name = args.dataset+'_p'+str(args.protein)
...
# p=10 protein family
test_idx = np.load(os.path.join(ROOT, 'data/graph/PDBbind2016/orphan_idx', '%d.npy'%args.protein))
train_idx = [i for i in range(4056)]
for i in test_idx: train_idx.remove(i) # 4056-811.2
...
adj_train = adj_all[train_idx] # 4056
