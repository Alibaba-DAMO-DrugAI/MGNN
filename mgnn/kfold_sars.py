...
LOG_PATH = os.path.join(ROOT, 'log', args.dataset+'_'+str(args.fold))
if not os.path.exists(MODEL_PATH): os.mkdir(MODEL_PATH)
if not os.path.exists(LOG_PATH): os.mkdir(LOG_PATH)
if not os.path.exists(LOG_PATH): os.mkdir(LOG_PATH)
shutil.copy('kfold_sars.py', LOG_PATH)
...
save_name = args.dataset+'_'+str(args.fold)
...
# Load data
TRAIN_PATH = os.path.join(ROOT, 'data/multi', '2019_%s'%args.dataset) # 17369
...
adj_mpro, feat_mpro, label_mpro = load_test(TEST_PATH)
# 185 / 5 = 37  k=5 k-fold
kfold_test_idx = np.load(os.path.join(ROOT, 'data/graph/SARS-CoV-BA/kfold_idx', '%d.npy'%args.fold))
kfold_train_idx = [i for i in range(185)]
for i in kfold_test_idx: kfold_train_idx.remove(i) # 185-37 = 148
adj_train = torch.cat([adj_train, adj_mpro[kfold_train_idx]], dim=0) # 17369 + 148
feat_train = torch.cat([feat_train, feat_mpro[kfold_train_idx]], dim=0)
label_train = torch.cat([label_train, label_mpro[kfold_train_idx]], dim=0)
adj_test = adj_mpro[kfold_test_idx] # 37
feat_test = feat_mpro[kfold_test_idx]
label_test = label_mpro[kfold_test_idx]
